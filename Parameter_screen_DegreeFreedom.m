...
function Parameter_screen_DegreeFreedom_052225(Extra_degree)
...
Pnames = {'k2','decB','decS','decBS','DB','DS','DBS','lambdaS',...
           'lambdaBS','j1','j2'};
% class(Extra_degree)
Extra_degree = str2num(Extra_degree);
% class(Extra_degree)
display(['Extra_degree is ' Pnames{Extra_degree}])
output_path = ['Result_200x5000/Result_Simple_' Pnames{Extra_degree}];
output_name = ['SSE_scenario_Simple_' Pnames{Extra_degree}];
folder_n = 200;
case_n = 5000;

%load parameters
load('ParaSet_base_200x5000.mat')
...
% change the d model parameter with extra degree of freedom
D_ALLP0 = Z_ALLP0;
if ~isempty(Extra_degree)
    Extra_P = zeros(sam_p,length(Extra_degree));
    Extra_P1 = zeros(sam_p,length(Extra_degree));
    
        for ed = 1:length(Extra_degree)
            screen_param = lhsdesign(sam_p,2, 'criterion','maximin', 'criterion','correlation'); 
            p_log = logspace(prange(Extra_degree(ed),1),prange(Extra_degree(ed),2),sam_p);
%             if Extra_degree(ed)<11
%                 edp = Extra_degree(ed)+1;
%             else
%                 edp = Extra_degree(ed)-1;
%             end
            Extra_P(:,ed)= p_log(round(screen_param(:,1).*sam_p));
            Extra_P1(:,ed)= p_log(round(screen_param(:,2).*sam_p));
            D_ALLP0(:,Extra_degree(ed))=Extra_P(:,ed);
            Z_ALLP0(:,Extra_degree(ed))=Extra_P1(:,ed);
        end
end
...
% ALLP0 = 10.^(ALLP0);
save(['./' output_path '/Parameters_screen'],'Z_ALLP0','D_ALLP0');   % screening parameters are saved
% toc 
