...
% Last Update: Thembi Mdluli04/26/2017
%
% functionality:
...
%   pars - vector of parameters that are varied
...
function [B, C, X, T] = FiniDiffMod2(n, tRange, parameters, pars)

tic; % start record time
%-------------Varying parameters
% binding rate was fixed at 0.004 for simple core model simulation
parameters.k2 = pars(1); % binding rates for BMP ligand and Chordin      nM^-1*s^-1 
parameters.k_2 = pars(1);  % unbinding rates for BMP ligand and Chordin    s^-1 
parameters.decB = pars(2); % decay rate of Bmp             s^-1
parameters.decC = pars(3); % decay rate of Chd             s^-1 
parameters.decBC = pars(4); % decay rate of Bmp-Chd        s^-1  
parameters.DB = pars(5); % diffusion rate of ligand (BMP)    (microns^2*s^-1)
parameters.DC = pars(6); % diffusion rate of Chordin         (microns^2*s^-1)
parameters.DBC = pars(7); % diffusion rate of [BMP,Chd]       (microns^2*s^-1)    
  %%% Tolloid behavior
parameters.lambda_tld_C = pars(8);% tld processing rate of Chd  nM^-1*m^-1
parameters.lambda_tld_BC = pars(9);% tld processing rate of LC   nM^-1*m^-1

%%% production rates
parameters.j1 = pars(10);        % production rate of BMP    s^-1 
parameters.j2 = pars(11);  % production rate of Chordin      s^-1

% nog parameters
% parameters.decN = pars(12);
% parameters.decBN = pars(13);
% parameters.DN = pars(14);
% parameters.DBN = pars(15);
% parameters.k_3 = pars(16);
% parameters.j3 = pars(17);
...
LdorN = parameters.LdorN;   % length of dorsal Nog expression region   microns
  %%% diffusion rates
DB = parameters.DB;       % diffusion rate of ligand (BMP)    (microns^2*s^-1)
DC = parameters.DC;       % diffusion rate of Chordin         (microns^2*s^-1)
DN = parameters.DN;       % diffusion rate of Noggin          (microns^2*s^-1)
DBC = parameters.DBC;     % diffusion rate of [BMP,Chd]       (microns^2*s^-1)
DBN = parameters.DBN;     % diffusion rate of [BMP,Nog]       (microns^2*s^-1)
  %%% kinetic parameters
k2 = parameters.k2;       % binding rates for BMP ligand and Chordin          nM^-1*s^-1
k_2 = parameters.k_2;     % unbinding rates for BMP ligand and Chordin        s^-1
k3 = parameters.k3;       % binding rates for BMP ligand and Noggin           nM^-1*s^-1
k_3 = parameters.k_3;     % unbinding rates for BMP ligand and Noggin         s^-1
  %%% decay/recycle rates 
decB = parameters.decB;   % decay rate of Ligand (BMP)    nM*s^-1  
decC = parameters.decC;   % decay rate of Chd             nM*s^-1  
decN = parameters.decN;   % decay rate of Nog             nM*s^-1  
decBC = parameters.decBC;   % decay rate of Chd             nM*s^-1  
decBN = parameters.decBN;   % decay rate of Nog             nM*s^-1  

  %%% production rates 
j1 = parameters.j1;       % production rate of BMP          nM*s^-1  
j2 = parameters.j2;       % production rate of Chordin      nM*s^-1
j3 = parameters.j3;       % production rate of Noggin       nM*s^-1
  %%% Tolloid behavior
lambda_tld_C = parameters.lambda_tld_C;     % tld processing rate of Chd  nM^-1*s^-1
lambda_tld_BC = parameters.lambda_tld_BC;   % tld processing rate of LC   nM^-1*s^-1
tld_conc = parameters.tld_conc;             % tld concentration           nM

  %%% Positive Feedback (turned off for the blastula simulation)
...
Tld = zeros(1,n);
x_lig = 1:n;
y = (exp(-(x_lig-11)/5))./(0.1+exp(-(x_lig-11)/5));
