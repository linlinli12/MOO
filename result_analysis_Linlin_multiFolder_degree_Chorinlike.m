...
%% Load data
% folder_path = 'MOO result/results/';
folder_path = 'main_code_linlin_P_front/Result_200x5000/';
save_folder = 'main_code_linlin_P_front/front_chordinlike_052825';    
...
save_output = 1 ; % check =1 if save all the SSE and WT and Chd/S mutant result 

nres =200;   %Screen file number in simple/Extended
n = 5000;    %Screen cases number in each file simple/Extended
...
for m = 0%:3% == Origianl screen'; '1 == 1D cases'; '2 == 2D cases'; '3 == 3D cases'
% datapath = './Result_120321_simple/SSE_scenario_Simple_';
    ...
    for f = 1:folder_n
        ...
        drosBd = zeros(nres*n,5);
        zebraBd = zeros(nres*n,12);
...
%                 Par(parcnt+folder_start_n:parcnt+n-1+folder_start_n,1:11)  = parameters_store;
        ...
        %     drosSH(parcnt:parcnt+n-1,:)  = Stored_Soln.dshet;     % BMP signal drosophila
            drosBd(parcnt:parcnt+n-1,:) = Bd(:,13:17);
            % zebrafish    
            zebraSSE(parcnt:parcnt+n-1,:)  = SSE(:,1:12);
            % Vard(parcnt:parcnt+n-1,:) = Bd;
            zebraBd(parcnt:parcnt+n-1,:) = Bd(:,1:12);
...
%     legend()
    ...
    save([save_folder '/Front_' folders_names{f} '.mat'],'ndx','ndy','ia','ndx1','ndy1','ia1','ndx2','ndy2','ia2','nond_drosWT','nond1_drosWT','nond2_drosWT',...
        'nond_zebraWT','nond1_zebraWT','nond2_zebraWT','nond_drosS','nond1_drosS','nond2_drosS','nond_zebraC','nond1_zebraC','nond2_zebraC','bd_dros','bd_zebra','bd_dros1','bd_zebra1','bd_dros2','bd_zebra2' )
    end
