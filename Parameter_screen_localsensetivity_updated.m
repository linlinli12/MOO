...
% function Parameter_screen_Run1case_052225(prameter_cases)
%% Drosophila Preliminaries 
clear;  clc; close all
%% Additional code
%%%%%%%%%% load Experimental results for zebrafish
load('./ExpData/ModeldataALL.mat');
addpath('./ExpData')
addpath('./Function');
% load('T57sym.mat');
Pnames = {'k2','decB','decS','decBS','DB','DS','DBS','lambdaS',...
           'lambdaBS','j1','j2'};
% load the best_parameter set as a local optimal to vary
load('best_parm_200x5000.mat')
prameter_cases = parameter_wtchd;
sam_p = 200;  % number of samples around the each paramter

% class(Extra_degree)
output_path = 'Result_singleCase';
output_name = 'Single_case';
...
Z_ALLP0 = [];
Z_ALLP0 = [Z_ALLP0;prameter_cases];

% parameter space for individual parameters
...
% screen_param = lhsdesign(sam_p,1, 'criterion','maximin', 'criterion','correlation'); 
% 
re_mat = repmat(prameter_cases,sam_p,1);

for k = 1:11
    re_mat = repmat(prameter_cases,sam_p,1);
    p_log = logspace(prange(k,1),prange(k,2),sam_p)';
    pii =p_log;%p_log(round(screen_param.*sam_p));
    re_mat(:,k) = pii;
    Z_ALLP0 = [Z_ALLP0;re_mat];
end

figure 
hold on
% run the local screen around best case
% run the best set first
[z_wt1,z_mu1,d_wt1,d_mu1] = Parameter_screen_Run1case(Z_ALLP0(1,:));


% Define base colors for each parameter
base_colors = lines(11);
scatter_handles = gobjects(11, 1);  % For legend

figure; hold on

for pi = 1:11
    base_color = base_colors(pi, :);
    
    % Get indices and parameter values for this pi
    param_idx_list = (1:sam_p) + (pi - 1) * sam_p + 1;
    param_values = Z_ALLP0(param_idx_list, pi);
    ref_value = Z_ALLP0(1, pi);  % Reference parameter value for comparison
    ...
    min_size = 50;
    max_size = 300;
    normalized_sizes = min_size + (param_values - min_val) / (max_val - min_val + eps) * (max_size - min_size);

    % Store points for connecting lines
    z_list = zeros(sam_p, 1);
    d_list = zeros(sam_p, 1);

    % Dummy invisible point for legend (triangle)
    scatter_handles(pi) = scatter(NaN, NaN, 150, ...
        '^', ...
    ...
    for ji = 1:sam_p
        idx = param_idx_list(ji);
        [z_wti, ~, d_wti, ~] = Parameter_screen_Run1case(Z_ALLP0(idx, :));

        z_list(ji) = z_wti;
        d_list(ji) = d_wti;

        % Determine marker size and type
        marker_size = normalized_sizes(ji);
        if Z_ALLP0(idx, pi) < ref_value
            marker_type = '.';
        else
            marker_type = '^';
        end

        % Scatter with chosen marker
        scatter(z_wti, d_wti, marker_size, ...
            marker_type, ...
            'MarkerFaceColor', base_color, ...
            'MarkerEdgeColor', base_color, ...
            'MarkerFaceAlpha', 0.9, ...
    ...
    % Connect all points for this parameter with a line
    plot(z_list, d_list, '-', 'Color', base_color, 'LineWidth', 1.5);
end

% Plot best parameter set
[z_wt1, ~, d_wt1, ~] = Parameter_screen_Run1case(Z_ALLP0(1, :));
best_handle = scatter(z_wt1, d_wt1, 300, 'r*', 'LineWidth', 4);
...
% Create legend
legend_handles = [scatter_handles; best_handle];
legend_labels = [Pnames, 'best parameter set'];
legend(legend_handles, legend_labels, 'Location', 'best');


function [z_wt,z_mu,d_wt,d_mu] = Parameter_screen_Run1case(prameter_cases)
load('./ExpData/ModeldataALL.mat');
load('experData');
addpath('./ExpData')
folder_n = 1;
case_n = size(prameter_cases,1);
...
load('./ExpData/chordinlike.mat'); % All chordilike sog data
temp{1} = load('./ExpData/WTData'); % All wildtype data
temp{2} = load('./ExpData/SogHomoData'); % All Sog homo data
temp{3} = load('./ExpData/SogHeterData'); % All Sog heter data
...
zparameters.LdorN = 78;   % length of dorsal Nog expression region microns
  %%% Positive Feedback (Turned of for simple model
zparameters.K = 0;%0/60;         % Vmax for positive feedback (multiply by 0 to turn off)
zparameters.kM = 0;%4000/60;       % kM parameter for Hill function
zparameters.nu = 0;%2;       % cooperative parameter
  %%% Noggin fixed parameters  
% load('./ExpData/parameters_zebra_noggin'); % Loard Noggin parameters previously optimized
%!!!!!!!!!!!!!!!!!! j3 = 0 for zebrafish simple model
zparameters.j3 = 0;%nondP(ind(1),30);  % production rate of Noggin       nM*s^-1
zparameters.k3 = 0;%nondP(ind(1),8);% binding rates for BMP ligand and Noggin          nM^-1*s^-1
zparameters.k_3 = 0;%nondP(ind(1),9); % unbinding rates for BMP ligand and Noggin          nM^-1*s^-1
zparameters.decN = 0;%nondP(ind(1),12);%0.15/60  * 10^(-2+4*p(7));    % decay rate of Nog             nM*m^-1
zparameters.decBN = 0;%nondP(ind(1),14);
zparameters.DN = 0;%nondP(ind(1),17);%10;       % diffusion rate of Noggin          (microns^2*s^-1)*60s*m^-1
zparameters.DBN = 0;%;nondP(ind(1),19);%6;       % diffusion rate of [BMP,Nog]       (microns^2*s^-1)*60s*m^-1  
...
% best parameter for simple model
Z_ALLP0 = prameter_cases;
D_ALLP0 = prameter_cases;
...
%     %-------------- screening operations ----------------
    nreg = case_n;
    p0_z= Z_ALLP0(cnt_par:cnt_par+nreg-1,:);    % select 2000 hundred parameters for each loop
    p0_d= D_ALLP0(cnt_par:cnt_par+nreg-1,:); 
...
%     SSE(:,2) = sum(((B_WT2 - repmat(pWT53,nreg,1))).^2,2); 
    ...
    % subplot(1,2,1) 
    % hold on
    % plot(B_WT3,'LineWidth',5)
    % hold on
    % plot(pWT57,'LineWidth',5)
    % % legend({'simulation WT','Exp 5.7hpf WT'},'Location','southeast','FontSize',12)
    % disp(['SSE WT = ' num2str(SSE(:,3))])
    % title (['Result compare for 5.7hpf SSE WT = ' num2str(SSE(:,3)) ])
    % 
...
%     SSE(:,7) = sum(((B_CLF2- repmat(pCLF53,nreg,1))).^2,2);
    ...
    z_mu = SSE(:,8);
%% plot 
    % plot(B_CLF3,'LineWidth',5)
    % hold on
    % plot(pCLF57/max(pWT57),'LineWidth',5)
    % legend({'simulation WT','Exp 5.7hpf WT','simulation CLF','Exp 5.7hpf CLF'},'Location','southeast','FontSize',12)
    % disp(['SSE CLF = ' num2str(SSE(:,3))])
    % title (['Result compare for 5.7hpf SSE CLF = ' num2str(SSE(:,5)) ])
...
%     ind = find(dst5);       dst4(dst5 == 0) = 0.1*dst5(ind(1)); 
    ...
    % subplot(1,2,2) 
    % hold on
    % plot(dB_WT/max(dB_WT),'LineWidth',5)
    % plot(dWT,'LineWidth',5)
    % plot(dB_SHomo/max(dB_SHomo)*0.5,'LineWidth',5)
    % plot(SHomot*0.5,'LineWidth',5)
    % legend({'simulation WT','Exp 5.7hpf WT','simulation CLF','Exp 5.7hpf CLF'},'Location','southeast','FontSize',12)
    % disp(['SSE CLF = ' num2str(SSE(:,13))])
    % title (['Result compare for 5.7hpf SSE CLF = ' num2str(SSE(:,5)) ])
    ...
    % selectd = [3,4;1,2;];
    % ysim = [Stored_Soln.dwt(1:36)/max(Stored_Soln.dwt(1:36)), Stored_Soln.zwt57(1:36)/max(Stored_Soln.zwt57(1:36));...  
    %     Stored_Soln.dshomo(1:36)/max(Stored_Soln.dshomo(1:36))*0.5, Stored_Soln.zclf57(1:36)/max(Stored_Soln.zclf57(1:36)),...
    %     ];
    % % selectd = [3,1]';
...
%     
%     fname=sprintf([output_path '/' output_name num2str(kj) '.mat']);
%     save(fname,'SSE','p0_d','p0_z','Stored_Soln','Bd');
%     kj=kj+1;
